...
1.  create package.json<br>
    `npm init --y`<br>
2.  create src folder berisi server.js & route.js<br>
3.  install hapi framework<br>
    `npm install @hapi/hapi`<br>
4.  running in terminal <br>
    `npm run start`<br>
    `then, you will see ‘Server berjalan pada http://localhost:9000’`<br>
5.  check request in server with cURL in Terminal/CMD<br>
    `curl -X GET http://localhost:9000/`<br>
6.  we can also check in browser, just click http://localhost:9000/ <br>
    Web server tersebut sudah berhasil merespons dan menampilkan data dalam dokumen HTML. Namun, web server yang kita buat belum mengenali sepenuhnya permintaan yang diberikan oleh client<br>
...
> /books/{bookId} --> path variable, kesatuan dengan endpoint (request.params)<br><br>
